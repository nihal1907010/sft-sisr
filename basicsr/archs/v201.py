...
class SWTForwardHaar2D(nn.Module):
    """
    Level-1 2D stationary wavelet transform (Haar).
    Padding: reflect (left/top only) to keep same HxW.
    Output: (ll, lh, hl, hh), each with shape like input.
    """
    def __init__(self):
        super().__init__()

    @staticmethod
    def _make_kernels(C, dtype, device):
        # 2x2 Haar analysis kernels (outer products of 1D h=[1,1]/sqrt(2), g=[1,-1]/sqrt(2))
        kLL = torch.tensor([[0.5,  0.5],
                            [0.5,  0.5]], dtype=dtype, device=device)  # low-low
        kLH = torch.tensor([[0.5, -0.5],
                            [0.5, -0.5]], dtype=dtype, device=device)  # low-high  (horizontal detail)
        kHL = torch.tensor([[0.5,  0.5],
                            [-0.5, -0.5]], dtype=dtype, device=device) # high-low  (vertical detail)
        kHH = torch.tensor([[0.5, -0.5],
                            [-0.5,  0.5]], dtype=dtype, device=device) # high-high

        # Depthwise weights (one 2x2 filter per channel)
        w_ll = kLL.view(1,1,2,2).repeat(C,1,1,1)
        w_lh = kLH.view(1,1,2,2).repeat(C,1,1,1)
        w_hl = kHL.view(1,1,2,2).repeat(C,1,1,1)
        w_hh = kHH.view(1,1,2,2).repeat(C,1,1,1)
        return w_ll, w_lh, w_hl, w_hh

    def forward(self, x: torch.Tensor):
        """
        x: (N, C, H, W)
        returns: ll, lh, hl, hh  (each (N, C, H, W))
        """
        if x.dim() != 4:
            raise ValueError(f"Expected 4D tensor (N,C,H,W), got {tuple(x.shape)}")
        N, C, H, W = x.shape
        if H < 2 or W < 2:
            raise ValueError("H and W must be >= 2 for reflect padding with a 2x2 Haar kernel.")

        w_ll, w_lh, w_hl, w_hh = self._make_kernels(C, x.dtype, x.device)

        # For even-sized kernel, pad (left=1, right=0, top=1, bottom=0) to keep same size
        xp = F.pad(x, (1, 0, 1, 0), mode='reflect')

        ll = F.conv2d(xp, w_ll, groups=C)
        lh = F.conv2d(xp, w_lh, groups=C)
        hl = F.conv2d(xp, w_hl, groups=C)
        hh = F.conv2d(xp, w_hh, groups=C)
        return ll, lh, hl, hh


class SWTInverseHaar2D(nn.Module):
    """
    Inverse of the level-1 2D Haar SWT used above.
    Given ll, lh, hl, hh (all same shape), reconstructs x.

    For this specific alignment, reconstruction is the closed-form:
        x = 0.5 * (ll - lh - hl + hh)
    (No padding or extra convolution needed.)
    """
    def __init__(self):
        super().__init__()
    ...
    def forward(self, ll: torch.Tensor, lh: torch.Tensor, hl: torch.Tensor, hh: torch.Tensor):
        if not (ll.shape == lh.shape == hl.shape == hh.shape):
            raise ValueError("All four subbands must have the same shape.")
        return 0.5 * (ll - lh - hl + hh)
...
#     torch.manual_seed(0)
#     device = "cpu"
#     dtype = torch.float64  # use double precision for a tight error bound

#     N, C, H, W = 2, 3, 64, 48
#     x = torch.randn(N, C, H, W, device=device, dtype=dtype)

#     fwd = SWTForwardHaar2D().to(device)
#     inv = SWTInverseHaar2D().to(device)
...
#     print(f"Reconstruction max |err|: {max_abs_err:.3e}")
#     print(f"Reconstruction MSE : {mse:.3e}")

#     # Optional: enforce a strict tolerance in double precision
#     assert max_abs_err < 1e-12, "Reconstruction error is larger than expected."

import torch
import torch.nn.functional as F

def swt_forward_haar2d(x: torch.Tensor):
    """
    Level-1 2D SWT (Haar), reflect padding (left/top only).
    x: (N, C, H, W)
    returns: (ll, lh, hl, hh) each (N, C, H, W)
    """
    if x.dim() != 4:
        raise ValueError(f"Expected 4D (N,C,H,W), got {tuple(x.shape)}")
    N, C, H, W = x.shape
    if H < 2 or W < 2:
        raise ValueError("H and W must be >= 2 for 2x2 Haar with reflect padding.")

    dtype, device = x.dtype, x.device
    with torch.no_grad():
        kLL = torch.tensor([[0.5,  0.5],
                            [0.5,  0.5]], dtype=dtype, device=device)
        kLH = torch.tensor([[0.5, -0.5],
                            [0.5, -0.5]], dtype=dtype, device=device)
        kHL = torch.tensor([[0.5,  0.5],
                            [-0.5, -0.5]], dtype=dtype, device=device)
        ...
        # Depthwise weights: (C, 1, 2, 2)
        w_ll = kLL.view(1, 1, 2, 2).repeat(C, 1, 1, 1)
        w_lh = kLH.view(1, 1, 2, 2).repeat(C, 1, 1, 1)
        w_hl = kHL.view(1, 1, 2, 2).repeat(C, 1, 1, 1)
        w_hh = kHH.view(1, 1, 2, 2).repeat(C, 1, 1, 1)

    # SAME size via reflect pad: (left=1, right=0, top=1, bottom=0)
    xp = F.pad(x, (1, 0, 1, 0), mode="reflect")
...
def swt_inverse_haar2d(ll: torch.Tensor, lh: torch.Tensor, hl: torch.Tensor, hh: torch.Tensor):
    """
    Inverse of the above level-1 2D Haar SWT.
    Given ll, lh, hl, hh (same shapes), reconstruct x:
        x = 0.5 * (ll - lh - hl + hh)
    """
    if not (ll.shape == lh.shape == hl.shape == hh.shape):
        raise ValueError("All four subbands must have the same shape.")
...
#     # Forward + Inverse SWT
#     ll, lh, hl, hh = swt_forward_haar2d(x)
#     x_rec = swt_inverse_haar2d(ll, lh, hl, hh)
...
#     assert max_abs_err < 1e-12, "Reconstruction error is larger than expected."
#     print("Test passed ✅ — Perfect reconstruction in double precision.")
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x1_path0 = x0_path0
        ll1_path1, lh1_path1, hl1_path1, hh1_path1 = swt_forward_haar2d(x0_path1)
        ...
        ll4_path0, lh4_path0, hl4_path0, hh4_path0 = swt_forward_haar2d(x3_path0)
        ll3_path1, lh1_path1, hl1_path1, hh1_path1 = self.mix_bands(self.freq_mix1, ll3_path1, lh1_path1, hl1_path1, hh1_path1)
        x4_path1 = swt_inverse_haar2d(ll3_path1, lh1_path1, hl1_path1, hh1_path1)
        ...
        x6_path0 = swt_inverse_haar2d(ll5_path0, lh4_path0, hl4_path0, hh4_path0)
        x6_path1 = x5_path1
