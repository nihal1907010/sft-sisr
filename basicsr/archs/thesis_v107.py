...
class BasicBlock(nn.Module):
    def __init__(self, dim, num_heads, hw, ww):
        ...
        self.freq_head1 = nn.Conv2d(dim // 2, 3 * dim // 2, 3, 1, 1)
        self.freq_head2 = nn.Conv2d(dim // 2, 3 * dim // 2, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        delta1 = self.freq_head1(ll1_path1)
        delta1 = delta1.reshape(b, 3, c // 2, h // 2 + h % 2, w // 2 + w % 2)
        lh0_path1 = lh0_path1 + delta1[:, 0]
        hl0_path1 = hl0_path1 + delta1[:, 1]
        hh0_path1 = hh0_path1 + delta1[:, 2]
        ...
        x2_path0 = x2_path0 + x0_path0
        ll2_path1 = ll2_path1 + ll0_path1
        ...
        delta2 = self.freq_head2(ll5_path0)
        delta2 = delta2.reshape(b, 3, c // 2, h // 2 + h % 2, w // 2 + w % 2)
        lh3_path0 = lh3_path0 + delta2[:, 0]
        hl3_path0 = hl3_path0 + delta2[:, 1]
        hh3_path0 = hh3_path0 + delta2[:, 2]

        # additional skip connection
        ll5_path0 = ll5_path0 + ll3_path0
        x5_path1 = x5_path1 + x3_path1
