...
SAVE_OVERLAY_CSV     = True       # save per-image metrics to CSV
# =========================


# ---------- Helpers ----------
def load_curve_csv(path: Path):
    """Return (freqs, power, cumulative) as numpy arrays."""
    df = pd.read_csv(path)
...
def main():
    ...
    for i, stem in enumerate(common, 1):
        try:
            ...
            rows.append({
                ...
                # quick indicators: who is closer overall
                "winner_power_mse": "Deep" if power_mse_deep < power_mse_classic else "Classic",
                "winner_cum_mse":   "Deep" if cum_mse_deep   < cum_mse_classic   else "Classic",
                "winner_high_band": "Deep" if (np.nan_to_num(high_mse_deep, nan=1e9) < np.nan_to_num(high_mse_classic, nan=1e9)) else "Classic",
            })
    ...
    summary = {
        ...
        "mean_dk50_classic": safe_mean("dk50_classic"),
        "mean_dk80_classic": safe_mean("dk80_classic"),
        "mean_dk50_deep": safe_mean("dk50_deep"),
        "mean_dk80_deep": safe_mean("dk80_deep"),
        "n_images": len(df),
        "deep_wins_power_mse_count": int((df["winner_power_mse"] == "Deep").sum()),
        "classic_wins_power_mse_count": int((df["winner_power_mse"] == "Classic").sum()),
        "deep_wins_cum_mse_count": int((df["winner_cum_mse"] == "Deep").sum()),
        "classic_wins_cum_mse_count": int((df["winner_cum_mse"] == "Classic").sum()),
        "deep_wins_high_band_count": int((df["winner_high_band"] == "Deep").sum()),
        "classic_wins_high_band_count": int((df["winner_high_band"] == "Classic").sum()),
    }
    ...
    # Win counts bar
    plt.figure(figsize=(7,5))
    wins = [summary["deep_wins_power_mse_count"], summary["classic_wins_power_mse_count"]]
    plt.bar(["Deep", "Classic"], wins)
    plt.title("Wins by Power MSE (per-image)")
    plt.ylabel("# Images won")
    plt.tight_layout()
    plt.savefig(OUT_DIR / "wins_power_mse.png", dpi=160)
    plt.close()

    print(f"Done. Per-image plots -> {figs_dir}, metrics -> {OUT_DIR}.")
