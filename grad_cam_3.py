...
# -*- coding: utf-8 -*-
"""
Grad-CAM over EVERY layer of your EDSR (BasicSR), with brighter grayscale maps:
- Normalize AFTER upsampling
- Percentile clipping + gamma (contrast boost)
- TURBO colormap (falls back to JET)
- Safe tensor-grad hooks; skips layers with no gradient path

Outputs:
  - sr.png
  - cam_gray_<layer>.png, cam_jet_<layer>.png, overlay_<layer>.png  (per-layer)
  - index.txt (marks SKIPPED when no grad captured)
...
# Visualization tuning (to avoid dark/black grayscale)
P_LOW, P_HIGH = 1.0, 99.0   # percentile clip
GAMMA = 0.6                 # <1 brightens; >1 darkens
FALLBACK_GRAY = 0.5         # used if dynamic range is ~0
CAM_RELU = True             # standard Grad-CAM (positive influence only)
...
def colorize_and_overlay(sr_rgb: np.ndarray, cam_01: np.ndarray, alpha=0.45):
    """
    sr_rgb: HxWx3 float [0,1]
    cam_01: HxW float [0,1]
...
def first_tensor(obj: Any) -> Optional[torch.Tensor]:
    """Pick a usable Tensor from Tensor/tuple/list/dict outputs."""
...
def postprocess_cam_01(cam_up: np.ndarray, p_low=1.0, p_high=99.0, gamma=0.6, fallback=0.5) -> np.ndarray:
    """
    cam_up: HxW float (not yet normalized)
    -> HxW float in [0,1], brightened and contrast-stretched
    """
    ...
    if hi - lo < 1e-8:
        return np.full_like(x, float(fallback), dtype=np.float32)
    x = (x - lo) / (hi - lo)
    x = np.clip(x, 0.0, 1.0)
    # gamma lift (gamma<1 brightens)
...
class GradCAM:
    ...
    Robust Grad-CAM:
      ...
      - Tensor-level grad hook is attached to capture gradients
    """
    ...
    def __call__(self, scalar: torch.Tensor, upsize_to: Tuple[int, int]) -> Optional[np.ndarray]:
        ...
        # Grad-CAM core (channel weights = GAP over spatial)
        weights = self.gradients.mean(dim=(2, 3), keepdim=True)            # (B,C,1,1)
        cam = (weights * self.activations).sum(dim=1, keepdim=True)        # (B,1,h,w)
        if CAM_RELU:
            cam = torch.relu(cam)

        # Upsample BEFORE normalization (then we normalize via percentiles)
        cam_up = F.interpolate(cam, size=upsize_to, mode="bilinear", align_corners=False)  # (B,1,H,W)
        cam_up_np = cam_up.detach().float().cpu().squeeze(0).squeeze(0).numpy()            # HxW float

        # Normalize & brighten
        cam_01 = postprocess_cam_01(cam_up_np, p_low=P_LOW, p_high=P_HIGH, gamma=GAMMA, fallback=FALLBACK_GRAY)
        return cam_01
...
def main():
    ...
    for idx, (name, module) in enumerate(targets, 1):
        ...
        # SR to numpy for overlays
        sr_np = sr.detach().cpu().squeeze(0).permute(1, 2, 0).numpy()
        sr_np = np.clip(sr_np, 0.0, 1.0)
        sr_np_cached = sr_np

        # Save grayscale (brightened) and color overlay
        gray3 = np.repeat(cam01[..., None], 3, axis=2)  # 3-ch gray for visibility
        heat_rgb, overlay_rgb = colorize_and_overlay(sr_np, cam01, alpha=0.45)
        ...
        index_lines.append(
            ...
            f"cam_color={os.path.basename(out_cam_jet)}  "
    ...
    with open(os.path.join(OUTDIR, "index.txt"), "w", encoding="utf-8") as f:
        f.write("Grad-CAM outputs per layer (percentile-normalized, gamma-lifted)\n")
